...
uint16_t getRawCellVoltage(BQ76920_t *BMS,int cell)
{
	if((cell == VC1)|| (cell == VC2)|| (cell == VC3) ||(cell == VC4))
	{
		...
		{
			case VC1:
				BMS->Vcell[0] = VoltageCellRaw;
				break;
			case VC2:
				BMS->Vcell[1] = VoltageCellRaw;
				break;
			case VC3:
				BMS->Vcell[2] = VoltageCellRaw;
				break;
			case VC4:
				BMS->Vcell[3] = VoltageCellRaw;
				break;
			default:
				break;
		}
		// return float value of requested cell
		return VoltageCellRaw;
	}
	else
		return  0;
}
...
uint16_t getRawPackVoltage(BQ76920_t *BMS)
{
	uint8_t regData[2] = {0u,0u};	// declare buffer
	BQ76920_ReadRegister(BMS, BAT_LO, &regData[0]);	// Read data in BAT_LO register
	BQ76920_ReadRegister(BMS, BAT_HI, &regData[1]); // Read data in BAT_HI register
...
short getRawCurrent(BQ76920_t *BMS)
{
	uint8_t regData[2] = {0u,0u};	// declare buffer
